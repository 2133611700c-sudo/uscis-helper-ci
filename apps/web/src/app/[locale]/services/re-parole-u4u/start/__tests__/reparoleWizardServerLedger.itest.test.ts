/**
 * P1 — Re-Parole server-ledger WIRING into the LIVE wizard (ReparoleWizardV2).
 *
 * Mirrors tpsWizardServerLedger.itest. The route + crypto + store are proven
 * elsewhere (wizard-draft/route.itest, wizardDraftCrypto.test,
 * wizardDraftStore.test). This suite proves the WIRING:
 *
 ...
 *     the shape ReparoleWizardV2 persists ({schema, uploadsMeta, lastStep, savedAt}).
 ...
 *  5. OFF-path parity (static): ReparoleWizardV2 gates EVERY ledger call on
...
// The exact persisted-draft shape ReparoleWizardV2 builds (PII-bearing).
const wizardDraft = {
  schema: 3,
  manual: {},
  packetReady: false,
  ...
  uploadsMeta: {
    passport: {
      ...
      canonical_document_id: 'canon-rp-789',
    },
  },
}
...
describe('Re-Parole server-ledger wiring — ON path (flag=1)', () => {
  beforeEach(() => {
    rows.clear()
  ...
  it('SAVE→HYDRATE roundtrip restores the exact draft (incl. canonical_document_id)', async () => {
    ...
    expect(await saveDraftToServer('reparole', wizardDraft, fetchImpl as never)).toBe(true)
    ...
    expect(restored?.uploadsMeta.passport.canonical_document_id).toBe('canon-rp-789')
  })

  it('browser keeps ONLY the opaque token cookie — no PII in the browser jar', async () => {
    const { fetchImpl, cookieJar } = makeBrowserFetch()
    await saveDraftToServer('reparole', wizardDraft, fetchImpl as never)
...
describe('ReparoleWizardV2 OFF-path parity (static guarantees)', () => {
  const SRC = readFileSync(
    join(__dirname, '..', 'ReparoleWizardV2.tsx'),
    'utf8',
  )
  ...
  it('SAVE: ledger POST is in the ON branch, localStorage.setItem in the OFF branch', () => {
    expect(SRC).toMatch(/if \(isLedgerClientEnabled\(\)\) \{\s*void saveDraftToServer\('reparole', draftRecord\)\s*\} else \{\s*localStorage\.setItem\(STORAGE_KEY, JSON\.stringify\(draftRecord\)\)/)
  })
  ...
  it('canonical_document_id is still persisted in the draft record (both paths)', () => {
    // uploadsMeta carries canonical_document_id in the persisted draft record.
    expect(SRC).toContain('canonical_document_id: u.canonical_document_id')
  })

  it('hydrate rebuild is shared via applyPersistedDraft (one rebuild for both paths)', () => {
    expect(SRC).toContain('const applyPersistedDraft = (parsed: unknown)')
  })
})

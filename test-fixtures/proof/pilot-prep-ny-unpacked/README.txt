Messenginfo — TPS Ukraine packet draft
Generated: 2026-05-11T07:08:07.761Z

WHAT THIS IS
  Two PDFs prefilled with the data you typed into the wizard:
    I-821 (Application for TPS) — edition 01/20/25, 32 AcroForm cells written
    I-765 (Application for EAD) — edition 08/21/25, 20 AcroForm cells written, category (a)(12)

WHAT TO DO
  1. Open each PDF in Adobe Acrobat Reader or Preview.
  2. Carefully review every prefilled field. Correct anything that is wrong.
  3. Complete every field that we could not fill (signature, certain Part 3/4 items).
  4. Print, sign in INK, and assemble your supporting documents.
  5. Pay the correct USCIS government fee (see below).
  6. Mail the package to the address shown below.

WHERE TO MAIL (resolved for state: NY)
  USCIS Chicago Lockbox

  By U.S. Postal Service:
    U.S. Citizenship and Immigration Services
    Attn: TPS Ukraine
    P.O. Box 4464
    Chicago, IL 60680-4464

  By FedEx, UPS, or DHL (street address — NOT for USPS):
    U.S. Citizenship and Immigration Services
    Attn: TPS Ukraine (Box 4464)
    131 S. Dearborn St., 3rd Floor
    Chicago, IL 60603-5517

  Source: https://www.uscis.gov/humanitarian/temporary-protected-status/TPS-Ukraine
  Snapshot date: 2026-05-10. Verify before mailing — addresses can change.

GOVERNMENT FEES (verify current amounts on uscis.gov before mailing)
  • I-821 — TPS application
    https://www.uscis.gov/g-1055?form=I-821
  • biometrics — Biometrics may apply (age 14+ generally)
    https://www.uscis.gov/g-1055
  • I-765 — Work permit (Employment Authorization Document)
    https://www.uscis.gov/g-1055?form=I-765
  Note: The exact dollar amounts above can change. Always verify the current fee on the official USCIS Fee Schedule (G-1055).

WHAT WE DID NOT DO
  - We did NOT submit anything to USCIS on your behalf.
  - We did NOT give you legal advice.
  - We did NOT determine your eligibility — please verify on uscis.gov.

SOURCE FORMS
  These PDFs were generated from the official USCIS PDFs verified against
  uscis.gov pages and PDF footer edition stamps on 2026-05-10.
  Official TPS Ukraine page: https://www.uscis.gov/humanitarian/temporary-protected-status/TPS-Ukraine
  See messenginfo.com/services/tps-ukraine/sources for all source links.

If a field looks wrong, do NOT mail the form. Edit it in Adobe first or
come back to messenginfo.com and re-run the wizard with corrected data.